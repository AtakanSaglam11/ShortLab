// Mock seed — 6 mois de données Instagram factices mais réalistes.
...
async function main() {
  ...
  // --- Account snapshots: 180 jours d'historique ---
  const DAYS = 180;
  let followers = 8500;
  let following = 320;
  ...
  for (let i = DAYS; i >= 0; i--) {
    ...
    // Dip artificiel sur les 14 derniers jours pour avoir une vraie alerte rouge
  ...
  // --- Snapshots quotidiens par vidéo (S-curve de croissance) ---
  ...
  // --- Conversions Shorzy: top 8 vidéos par qualité ---
  ...
  await prisma.syncRun.create({
    data: {
      ...
      status: "success",
      finishedAt: new Date(),
      notes: `Seed initial: ${videos.length} vidéos, ${DAYS} jours d'historique`,
    },
  });

  console.log(
    `✅ Seed done — 1 account, ${videos.length} videos, ${DAYS} days of history, ${top.length * 2} conversions`,
  );
}
